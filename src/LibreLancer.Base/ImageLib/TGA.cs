...
public static class TGA
{
    ...
    public static Image ImageFromStream(Stream stream, int channels = 0)
    {
        ...
        var bytesPerPixel = BytesPerPixel(header.Indexed == 1 ? header.PaletteBits : header.BitsPerPixel);
        var tgaData = new byte[header.Width * header.Height * bytesPerPixel];
        ...
        else
        {
            ...
            var readNextPixel = false;
            var rleCount = 0;
            var rleRepeating = false;
